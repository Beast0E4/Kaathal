      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        <div className="flex items-center justify-between mb-12">
          <div>
            ...
            <p className="text-gray-500">Manage your blog posts and drafts.</p>
          </div>
          <Link to={'/blog'}
            ...
            {posts.map(post => (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <span className={`px-2 py-1 rounded-md text-xs font-medium ${post.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-yellow-50 text-yellow-700'}`}>
                    {post.status === 'published' ? 'Published' : 'Draft'}
                  </span>
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                      onClick={(e) => { e.stopPropagation(); navigate('/post', { id: post.id }); }}
                ...
                <h3 className="font-bold text-xl text-slate-900 mb-2 line-clamp-2">{post.title || "Untitled Story"}</h3>
                ...
                <div className="text-xs text-gray-400 pt-4 border-t border-gray-50 flex justify-between items-center">
                  <span>{post.updatedAt ? new Date(post.updatedAt.seconds * 1000).toLocaleDateString() : 'Just now'}</span>
