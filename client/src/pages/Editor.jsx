...
function Editor() {
    ...
    const insertFormat = (type) => {
        ...
        // Interrupt flow for image URL input
        if (type === 'image') {
        ...
        if (formats[type]) {
            ...
            if (f.wrap) {
                newText =
                    ...
                    `${f.wrap}${selectedText}${f.wrap}` +
                    text.substring(end);
                newCursorPos = end + f.offset;
            } else {
                newText =
                    text.substring(0, start) +
                    `${f.prefix}${selectedText}` +
                    text.substring(end);
                newCursorPos = end + f.offset;
            }
        }
                ...
                <main className="flex-1 overflow-y-auto relative">
                    <div className="max-w-3xl mx-auto py-12 px-8 min-h-[calc(100vh-4rem)] bg-white my-8 rounded-xl shadow-sm border border-gray-100 relative">
                        ...
                        {!cover_image ? (
                            ...
                            <div className="relative h-64 w-full mb-8 rounded-xl overflow-hidden group">
                                ...
                                <button
                                    ...
                                    className="absolute top-4 right-4 bg-white/90 p-2 rounded-full text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity shadow-md hover:text-red-500"
                                >
                                    <X size={18} />
                                </button>
                            </div>
                        ...
                        <input
                            ...
                            className="w-full text-5xl font-bold text-slate-900 placeholder-gray-300 border-none focus:ring-0 focus:outline-none mb-6 tracking-tight leading-tight"
                        />
                        ...
                        <textarea
                            ...
                            className="w-full resize-none text-xl leading-relaxed text-gray-700 border-none focus:ring-0 focus:outline-none min-h-[400px] font-serif"
                            spellCheck={false}
                        />
                    </div>
                </main>
                            ...
                            <div className="flex items-center bg-gray-50 rounded-lg border border-gray-200 transition-all">
                                ...
                                <input
                                    ...
                                    placeholder={title
                                        .toLowerCase()
                                        .replace(/\s+/g, '-')}
                                    className="w-full bg-transparent border-none text-sm p-3 text-gray-700 focus:ring-0"
                                />
                            </div>
                        </div>
                        ...
                        <div className="mb-6">
                            ...
                            <div className="flex flex-wrap gap-2 mb-2">
                                {tags.map((tag) => (
                                    <span
                                        key={tag}
                                        className="px-2 py-1 bg-slate-100 text-slate-600 text-xs rounded-md flex items-center gap-1"
                                    >
                                        {tag}
                                        <button
                                            onClick={() =>
